...
version (BindRaylib_Static) {
   ...
   static if (raylibSupport > RaylibSupport.raylib300_70) {
      ...
      // 300, 300_70
      extern (C) @nogc nothrow {
         alias prlEnableRenderTexture = void function(uint id); // Enable render texture (fbo)
         alias prlDisableRenderTexture = void function(); // Disable render texture (fbo), return to default framebufferP
         alias prlDeleteTextures = void function(uint id); // Delete OpenGL texture from GPU
         alias prlDeleteRenderTextures = void function(RenderTexture2D target); // Delete render textures (fbo) from GPU
         alias prlDeleteShader = void function(uint id); // Delete OpenGL shader program from GPU
         alias prlDeleteVertexArrays = void function(uint id); // Unload vertex data (VAO) from GPU memory
         alias prlDeleteBuffers = void function(uint id); // Unload vertex data (VBO) from GPU memory
         alias prlUnproject = Vector3 function(Vector3 source, Matrix proj, Matrix view); // Get world coordinates from screen coordinates
         alias prlLoadTextureDepth = uint function(int width, int height, int bits, bool useRenderBuffer); // Load depth texture/renderbuffer (to be attached to fbo)
         alias prlUpdateTexture = void function(uint id, int width, int height, int format, const(void)* data); // Update GPU texture with new data
         alias prlLoadRenderTexture = RenderTexture2D function(int width, int height, int format, int depthBits, bool useDepthTexture); // Load a render texture (with color and depth attachments)
         alias prlRenderTextureAttach = void function(RenderTexture target, uint id, int attachType); // Attach texture/renderbuffer to an fbo
         alias prlRenderTextureComplete = bool function(RenderTexture target); // Verify render texture is complete
      }
